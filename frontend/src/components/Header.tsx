...
export function Header({ activeTab, hasCart }: HeaderProps) {
  return (
    <header className="border-b border-white/5 bg-[#0A0A0A]/80 backdrop-blur-xl sticky top-0 z-40">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 flex gap-4 sm:gap-6 text-sm font-medium border-t border-white/5 overflow-x-auto no-scrollbar">
        <Link
          to="/"
          className={`py-3 whitespace-nowrap transition-colors ${activeTab === '/' ? 'text-white border-b-2 border-emerald-500' : 'text-gray-500 border-b-2 border-transparent hover:text-gray-300'}`}
        >
          Box Office
        </Link>
        <Link
